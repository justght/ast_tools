...
//js混淆代码读取
process.argv.length > 2 ? encodeFile = process.argv[2]: encodeFile ="./ifob.js";
	...
	SwitchStatement(path)
	{
		...
		path.traverse({  //学习在path下的遍历，这个方法可以减少ast这个实参
			AssignmentExpression(_path)
			{
				...
				if (!parentPath.isExpressionStatement({"expression":node}))//并且它的 expression 属性是否与当前节点 node 相同。
				{
					return;
				}
